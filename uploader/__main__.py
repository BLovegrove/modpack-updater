...
def upload_file(filepath_local: str, filename_remote: str):
	...
	with paramiko.SSHClient() as ssh:
		...
		try:
			with tqdm(desc='Progress ', unit='iB', unit_scale=True, unit_divisor=1024) as pbar:
...
def main():
	...
	version_old = common.version_decode(cfg['pack']['version'])
	while True:
		version_new = input(
			os.linesep +
	...
	upload_file("upload_temp/modpack.zip", f"modpack_latest.zip")
